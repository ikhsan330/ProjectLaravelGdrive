{{-- File: _folder_contents.blade.php (Desain Rapi dengan Ikon Spesifik) --}}
<div class="pb-4 mb-6 border-b border-gray-200">
    <h3 class="text-2xl font-bold text-gray-900">{{ $selectedFolder->name }}</h3>
    @if($selectedFolder->user)
        <p class="text-sm text-gray-500 mt-1">
            Pemilik: <span class="font-semibold text-gray-700">{{ $selectedFolder->user->name }}</span>
    ...
    @if($documents->isNotEmpty())
        <ul class="space-y-3">
            @foreach($documents as $document)
                ...
                @endphp
                <li>
                    <a href="#" class="flex items-center p-4 rounded-xl hover:bg-gray-50 transition-colors duration-200 border border-gray-200 group">
                        <svg class="w-8 h-8 mr-4 flex-shrink-0 {{ $icon_color }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $icon_path }}" /></svg>

                        <div class="flex-grow">
                            <p class="font-medium text-gray-800 group-hover:text-blue-600">{{ $document->name ?? 'Tanpa Judul' }}</p>
