...
@section('content')
    {{-- Bagian banner dan konten lainnya bisa diletakkan di sini --}}
        ...
        <div class="max-w-7xl mx-auto">
            <div class="text-center mb-12">
                ...
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    Jelajahi struktur folder publik dan temukan dokumen yang Anda cari dengan mudah.
                </p>
            </div>
                ...
                <div class="w-full md:w-2/5 lg:w-1/3">
                    <div class="bg-white p-4 sm:p-6 rounded-2xl shadow-md border border-gray-100 h-full">
                        ...
                        <ul id="folder-tree-container" class="space-y-1">
                            {{-- Memuat sidebar dengan data folder induk --}}
                            @include('partials._folder_sidebar', ['rootFolders' => $rootFolders])
                        </ul>
                    </div>
                </div>

                {{-- Panel Kanan: Tampilan Konten Folder --}}
...
<script>
    // Script Anda tidak perlu diubah, karena sudah berfungsi dengan baik untuk memuat konten via AJAX.
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Menambahkan event listener ke area dokumen juga agar subfolder yang di-load bisa diklik
        const clickHandler = function(e) {
            const target = e.target.closest('.folder-item');
            if (!target) return;

            // Toggle sub-list di sidebar kiri
            ...
            if (folderId) {
                ...
                fetch(url)
                    ...
                    .catch(error => {
                        ...
                        documentArea.innerHTML = `<div class="text-center p-10 text-red-500 font-medium"><p>Gagal memuat konten. Silakan coba lagi.</p></div>`;
                    });
            }
        };

        folderContainer.addEventListener('click', clickHandler);
        documentArea.addEventListener('click', clickHandler);
    });
</script>

    {{-- Bagian lain dari halaman welcome, seperti features --}}
