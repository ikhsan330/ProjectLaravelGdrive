<x-app-layout>
    <div class="container py-4">
        ...
        <nav aria-label="breadcrumb" class="mb-3">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('p4pm.dokumen.index') }}">Tinjauan Dokumen P4PM</a></li>
                @foreach ($breadcrumbs as $crumb)
                    <li class="breadcrumb-item"><a href="{{ route('p4pm.folder.show', $crumb->folder_id) }}">{{ $crumb->name }}</a></li>
                @endforeach
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        @foreach($subfolders as $subfolder)
                            <li class="list-group-item list-group-item-action d-flex flex-wrap align-items-center justify-content-between gap-3 py-3">
                                ...
                                <a href="{{ route('p4pm.folder.show', $subfolder->folder_id) }}" class="btn btn-outline-primary btn-sm">
                        ...
                        @foreach ($documents as $document)
                            <li class="list-group-item d-flex flex-wrap align-items-center gap-3 py-3">
                                ...
                                <div class="btn-group flex-shrink-0" role="group">
                                    <a href="{{ route('p4pm.document.preview', $document->id) }}" target="_blank" class="btn btn-outline-primary btn-sm" title="Lihat Dokumen">
                                        <i class="bi bi-eye"></i> Lihat
                                    </a>
                                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#commentsModal-{{ $document->id }}">
                                        <i class="bi bi-chat-left-text"></i> Komentar
                                        @if($document->comments->count() > 0)
                                            <span class="badge bg-light text-dark ms-1">{{ $document->comments->count() }}</span>
                                        @endif
                                    </button>
                                </div>
                            </li>
            ...
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form action="{{ route('p4pm.document.comment.store', $document->id) }}" method="POST">
                            @csrf
                            <div class="mb-3">
